...
function createErrorBlock(error: string, status?: number, userData?: { user: any, workspace: any } | null): HTMLElement {
  const block = document.createElement("div");
  block.id = "pimms-analytics-block";
  block.style.cssText =
  ...
  const userHeaderHTML = userData ? createUserHeaderHTML(userData.user, userData.workspace) : '';
  ...
  if (config?.analyticsPageUrlPattern) {
    try {
      ...
      if (!re.test(window.location.href)) {
        logger.debug('[PIMMS][Analytics] URL did not match analyticsPageUrlPattern, skip');
        return;
      }
    } catch {
        ...
        const onResult = async (message: any) => {
          ...
          try {
            ...
            if (message.ok === false && message.error) {
              logger.warn('[PIMMS][Analytics] API error received:', message.error);
              const errorBlock = createErrorBlock(message.error, message.status, cachedUserData);
