    ...
    if (this.detectionEnabled) {
      this.linkDetector = new LinkDetector((links) => {
        if (window.pimmsPanelApp) {
          window.pimmsPanelApp.updateLinks(links);
        }
      ...
      const tryEmit = () => {
        ...
        if (window.pimmsPanelApp?.updateLinks) {
          window.pimmsPanelApp.updateLinks(links);
        } else {
      ...
      this.hoverHandler.onHoveredLink = (link) => {
        ...
        const linkStillExists = currentLinks.find(
          (currentLink) => currentLink.id === link.id,
        );

        if (window.pimmsPanelApp && linkStillExists) {
          window.pimmsPanelApp.showHoveredLink(link);
        }
      };

      this.hoverHandler.onHideHoveredLink = () => {
        if (window.pimmsPanelApp) {
          window.pimmsPanelApp.hideHoveredLink();
        }
      };
    }
...
if (isEmailMarketingDomain(window.location.hostname)) {
  ...
  const manageAnalytics = (shouldShow: boolean) => {
    ...
    if (shouldShow) {
      ...
      if (!existingBlock) {
        // Get user email from the panel API (already available from React components)
        const userEmail = window.pimmsPanelApp.getUserEmail();
        if (!userEmail) {
          console.warn(
            "[PIMMS] No user email available, skipping analytics injection",
          );
          return;
        }
        ...
        tryInjectAnalyticsPreview(normalizedHost, userEmail, scope).catch(
          () => {
  ...
  const ensureInit = async (isLoggedIn?: boolean) => {
    ...
    const authStatus =
      isLoggedIn !== undefined ? isLoggedIn : lastKnownAuthState ?? false;
    ...
    if (active && detectionEnabled === shouldEnableDetection) {
      ...
      if (shouldEnableDetection) {
        ...
        setTimeout(() => {
          const links =
            (active as any)?.linkDetector?.getCurrentLinks?.() || [];
          if (window.pimmsPanelApp?.updateLinks) {
            window.pimmsPanelApp.updateLinks(links);
          }
        }, 50);
  ...
  window.pimmsEnsureInit = ensureInit;
  ensureInit();
