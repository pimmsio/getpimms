...
export const renderPanel = () => {
  ...
  const loadCss = async () => {
    try {
      ...
      const cacheMode = process.env.NODE_ENV === 'development' ? 'no-store' : 'default';
  ...
  if (process.env.NODE_ENV === 'development' && process.env.REACT_APP_DISABLE_CSS_POLLING !== 'true') {
    let lastCss = '';
    const poll = async () => {
      try {
        const res = await fetch(`${cssUrl}?t=${Date.now()}`, { cache: 'no-store' });
        if (!res.ok) return;
