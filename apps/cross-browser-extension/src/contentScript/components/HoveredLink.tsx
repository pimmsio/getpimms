...
const HoveredLink: React.FC<HoveredLinkProps> = memo(
  ({
    ...
    // State for currently selected domain for this action (separate from default domain)
    const [currentlySelectedDomain, setCurrentlySelectedDomain] = useState<string | null>(null);
    
    // Initialize with defaultDomain when it becomes available
    React.useEffect(() => {
      if (!currentlySelectedDomain && defaultDomain) {
        setCurrentlySelectedDomain(defaultDomain);
      }
    }, [defaultDomain, currentlySelectedDomain]);
    ...
    const handleShortenClick = useCallback(async () => {
      const success = await onShortenClick(link.href, position, currentlySelectedDomain || undefined);
      if (isInCache && success) {
        // For cached links, show copied feedback immediately on success
        setJustCopied(true);
        setTimeout(() => setJustCopied(false), 2000);
      }
    }, [link.href, position, onShortenClick, isInCache, currentlySelectedDomain]);
    ...
    }, [link.href, position, onShortenClick, currentlySelectedDomain]);
          ...
          {!hasShortened && !isInCache && !isAlreadyPimms && (
            <div>
              ...
              {domainsLoading ? (
                ...
                <DomainSelector
                  domains={domains}
                  selectedDomain={currentlySelectedDomain}
                  onDomainChange={(domain) => {
                    console.log('🔥 HoveredLink received domain change:', domain);
                    console.log('🔥 Previous currentlySelectedDomain:', currentlySelectedDomain);
                    setCurrentlySelectedDomain(domain);
                    console.log('🔥 HoveredLink domain set to:', domain);
                  }}
                  defaultDomain={defaultDomain}
                  onSetDefaultDomain={setDefaultDomain}
