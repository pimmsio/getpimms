  ...
  return (
    <div style={{
      ...
      <div style={{ color: "#991b1b", marginBottom: "8px" }}>
        Cannot connect to PIMMS. Please check your connection and reload the page.
      </div>
      {error && (
...
const PanelApp: React.FC = () => {
  ...
  const isPanelActive = Boolean((isLoggedIn || (authState.isConnectionError && user)) && hasRootSelectors);
  
  // Debug logging
  console.log('[PANEL] Auth state:', { 
    status: authState.status, 
    isConnectionError: authState.isConnectionError, 
  ...
  useEffect(() => {
    if (authState.status !== "unknown") {
      ...
      if ((window as any).pimmsEnsureInit) {
        (window as any).pimmsEnsureInit(loggedIn);
      }
    }
  }, [authState.status]);
  ...
  }), [setIsVisible, resetPanelState, closedUntilReloadRef, setPanelState, overlay, setHoveredLink, detailLockedRef, setHoveredLinkPosition]);
