...
import { extractCampaignId } from "../utils/extractCampaignId";

export async function sendChromeMessage(
  message: any,
  timeoutMs: number = 4000,
): Promise<{ __error?: string; __timeout?: boolean; resp?: any }> {
...
export function extractUtmForCurrentHost(href: string, userEmail: string) {
  ...
  const cfg = EMAIL_MARKETING_DOMAINS.find(
    (d) => host === d.domain || host.endsWith(`.${d.domain}`),
  );

  // Extract campaign ID from current page URL
  const campaignFromUrl = extractCampaignId(cfg, userEmail);

  const truncate = (str: string | null | undefined) =>
    str?.slice(0, 190) ?? null;

  return {
    utm_source: truncate(
      params?.get("utm_source") ?? cfg?.defaultUtmSource ?? null,
    ),
    utm_medium: truncate(
      params?.get("utm_medium") ?? cfg?.defaultUtmMedium ?? null,
    ),
    utm_campaign: truncate(params?.get("utm_campaign")),
    utm_term: truncate(params?.get("utm_term")),
    utm_content: truncate(params?.get("utm_content") ?? campaignFromUrl),
  };
}
...
export async function shortenViaOffscreen(
  href: string,
  domain?: string,
  ...
  // Get user email for better campaign ID generation from panel API
  const userEmail = window.pimmsPanelApp.getUserEmail();
  const { utm_source, utm_medium, utm_campaign, utm_term, utm_content } =
    extractUtmForCurrentHost(href, userEmail);
