...
// Single flame icon component
export function SingleFlameIcon({
  className = "w-6 h-6",
  score = 0,
}: {
  className?: string;
  ...
  // Color based on score: cold (blue), warm (orange), hot (red)
  const getColor = (score: number) => {
    if (score <= 33) {
      return { fill: "#2FCDFA", bg: "#C0F1FF" }; // Cold - blue
    } else if (score <= 66) {
      return { fill: "#F59F00", bg: "#FEF3C7" }; // Warm - orange
    } else {
      return { fill: "#F0174A", bg: "#FECACA" }; // Hot - red
    }
  };
...
export function getHotScoreIcon(score: number) {
  // Return SingleFlameIcon component with score bound
  return SingleFlameIcon;
}
