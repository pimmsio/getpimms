...
const LinkCardInner = memo(
  ({
    ...
    const handleRowClick = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const modalBackdropPresent =
        typeof document !== "undefined" &&
        Boolean(document.getElementById("modal-backdrop"));

      const isFromRadixDialog = Boolean(target?.closest('[role="dialog"]'));

      // If a modal is open, Radix Dialog events can still bubble through React to this row.
      // Never navigate on row click in that case.
      if (modalBackdropPresent || isFromRadixDialog) {
        return;
      }
          ...
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-3 rounded-md px-2 py-2 text-sm transition-[box-shadow] group-hover:ring-1 group-hover:ring-neutral-200/60 sm:px-5 sm:py-3">
            <div ref={ref} className="min-w-0 overflow-hidden">
              ...
              <div className="mt-2 hidden min-w-0 flex-col gap-2 xl:hidden">
                {utmVisibility?.visibleUtmKeys?.length ||
                utmVisibility?.showTagsColumn ? (
