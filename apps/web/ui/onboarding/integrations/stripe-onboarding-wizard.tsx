...
export function StripeOnboardingWizard({
  ...
  useEffect(() => {
    if (!done) return;
    if (completedProviderIds.includes("stripe")) return;
    void setCompletedProviderIds([...completedProviderIds, "stripe"]);
  ...
  const startConnectFlow = useCallback(async () => {
    ...
    try {
      ...
      const installUrl = typeof data?.installUrl === "string" ? data.installUrl : null;
      if (installUrl) {
            ...
            <ManualConfirmStep
              ...
              actions={
                <button
                  ...
                  disabled={connecting || stripeInstalled}
                >
                  {connecting ? (
                    <>
