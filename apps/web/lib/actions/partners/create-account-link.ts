...
export const createAccountLinkAction = authPartnerActionClient.action(
  async ({ ctx }) => {
    ...
    if (!partner.stripeConnectId) {
      ...
      if (!partner.country) {
        throw new Error(
          "You haven't set your country yet. Please go to partners.pimms.io/settings to set your country.",
        );
      }
      ...
      if (!CONNECT_SUPPORTED_COUNTRIES.includes(partner.country)) {
        throw new Error(
          `Your current country (${COUNTRIES[partner.country]}) is not supported for Stripe Connect. Please go to partners.pimms.io/settings to update your country, or contact support.`,
        );
      }
