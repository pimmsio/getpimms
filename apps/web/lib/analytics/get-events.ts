...
export const getEvents = async (params: EventsFilters) => {
  ...
  // Fetch both leads and sales events
  const leadsPipe = tb.buildPipe({
    pipe: "v2_events",
    parameters: eventsFilterTB,
    data: leadEventSchemaTBEndpoint,
  });

  const salesPipe = tb.buildPipe({
    pipe: "v2_events",
    parameters: eventsFilterTB,
    data: saleEventSchemaTBEndpoint,
  });
  ...
  const [leadsResponse, salesResponse] = await Promise.all([
    leadsPipe({
      ...
      region,
      order: sortOrder,
      offset: 0, // Get all results, we'll sort and paginate after merge
      limit: params.limit * 2, // Get more to ensure we have enough after merge
  ...
  const combinedData = [...leadsResponse.data, ...salesResponse.data]
    .sort((a, b) => {
      const aTime = new Date(a.timestamp).getTime();
      const bTime = new Date(b.timestamp).getTime();
      return sortOrder === "asc" ? aTime - bTime : bTime - aTime;
    })
    .slice((params.page - 1) * params.limit, params.page * params.limit); // Apply pagination
      ...
      const eventData = {
        ...
        ...(evt.event === "lead" || evt.event === "sale"
          ? {
              ...
              ...(evt.event === "sale"
                ? { invoice_id: evt.invoice_id, payment_processor: evt.payment_processor }
                : {}),
            }
          : {}),
      };
