...
import { getStoredExtensionId } from "../extension-id-handler";
  ...
  useEffect(() => {
    ...
    const attemptExtensionCommunication = async () => {
      ...
      try {
        ...
        // Get extension ID from local storage (persisted from initial URL)
        const extensionId = getStoredExtensionId() || process.env.NEXT_PUBLIC_CHROME_EXTENSION_ID;
        console.log('[CBE] Using extension ID:', extensionId);
    ...
    <div className="mx-auto w-full max-w-md">
      <div className="flex flex-col items-center justify-center space-y-6">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="w-full rounded-lg bg-yellow-50 px-4 py-3 text-center">
            <p className="text-xs text-yellow-800">
              Debug: Extension ID = {getStoredExtensionId() || process.env.NEXT_PUBLIC_CHROME_EXTENSION_ID || 'Not found'}
            </p>
          </div>
        )}
      </div>
