...
export default async function dashboardPage({
  ...
  if (
    data.password &&
    cookies().get(`pimms_password_${params.dashboardId}`)?.value !== data.password
  ) {
    return (
      <main className="flex h-screen w-screen items-center justify-center">
        <NewBackground />
        <div className="z-10 w-full max-w-md overflow-hidden rounded border border-neutral-100 shadow-xl">
          <div className="flex flex-col items-center justify-center space-y-3 border-b border-neutral-100 bg-white px-4 py-6 pt-8 text-center sm:px-16">
            <Logo />
