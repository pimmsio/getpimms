  ...
  return (
    <div className="flex flex-col space-y-5">
      <form
        action={async (data) =>
          await fetch(
            `/api/admin/links/ban?domain=pim.ms&key=${data.get("key")}`,
            {
              method: "DELETE",
            },
          )
  ...
  return (
    <div className="relative flex w-full rounded shadow-sm">
      ...
      <input
        ...
        autoComplete="off"
        className={cn(
          "block w-full rounded-r-md border-neutral-300 text-sm text-neutral-900 placeholder-neutral-400 focus:border-neutral-500 focus:outline-none focus:ring-0",
        ...
        onPaste={(e: React.ClipboardEvent<HTMLInputElement>) => {
          ...
          if (
            text.startsWith("https://pim.ms/") ||
            text.startsWith("pim.ms/")
          ) {
            e.currentTarget.value = text
              .replace("https://pim.ms/", "")
              .replace("pim.ms/", "");
          } else {
            e.currentTarget.value = text;
          }
