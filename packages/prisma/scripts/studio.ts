...
function sanitizeDatabaseUrl(rawUrl: string): string {
  ...
  // PlanetScale requires SSL/TLS. MySQL2 (used by Prisma Studio tooling) ignores `sslaccept`,
  // so enforce TLS using an `ssl` profile object instead of a boolean.
  try {
...
const rawUrl = process.env.DATABASE_URL ?? process.env.PLANETSCALE_DATABASE_URL ?? "";
if (!rawUrl) {
  console.error(
    "Missing DATABASE_URL/PLANETSCALE_DATABASE_URL. Prisma Studio requires a database URL env var to be set (both are currently empty).",
  );
  process.exit(1);
}
...
try {
  ...
  console.log(
    `[prisma:studio] DB: ${u.protocol}//${u.host}${u.pathname} (tlsParams=${hasTlsHint})`,
  );
} catch {
  // ignore
}

// Forward any args passed to `pnpm ... studio -- <args>`
...
const child = spawn(
  ...
  ],
  {
    ...
    env: {
      ...process.env,
      // Keep env consistent with the passed `--url`, in case Prisma/Studio reads it too.
